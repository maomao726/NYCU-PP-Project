...
Mat CourtLinePixelDetector::run(const Mat& frame, const std::string& c)
{
    Mat trans = frame.clone();
    trans = colorTransfer(frame, c);
  TimeMeasurement::start("CourtLinePixelDetector::run");

  TimeMeasurement::start("\tgetLuminanceChannel");
  Mat luminanceImage = getLuminanceChannel(trans);
...
cv::Mat CourtLinePixelDetector::getLuminanceChannel(const cv::Mat& frame)
{
  ...
  if (debug)
  {
    ...
    displayImage("A", luminanceChannel);
  }
...
Mat CourtLinePixelDetector::detectLinePixels(const cv::Mat& image)
{
  ...
  Mat element = getStructuringElement(MORPH_RECT, Size(parameters.morphology_size, parameters.morphology_size));
  morphologyEx(pixelImage, pixelImage, MORPH_CLOSE, element);
  ...
  for (unsigned int x = 0; x < image.cols; ++x)
  {
      for (unsigned int y = 0; y < image.rows; ++y)
      {
          pixelImage.at<uchar>(y, x) = isLinePixel(pixelImage, x, y);
      }
  }
  if (debug)
  {
...
{
  Mat dx2, dxy, dy2;
  computeStructureTensorElements(binaryImage, dx2, dxy, dy2);
...
Mat CourtLinePixelDetector::colorTransfer(const Mat& frame, const std::string& color_mode)
{
    Mat imgtrans;
    cvtColor(frame, imgtrans, CV_RGB2HSV);

    std::vector<Mat> channel(3);
    split(imgtrans, channel);
    if (color_mode == "yellow")
    {
        for (int i = 0; i < imgtrans.cols; i++)
        {
            for (int j = 0; j < imgtrans.rows; j++)
            {
                channel[0].at<uchar>(j, i) = (channel[0].at<uchar>(j, i) - 30) % 180;
    ...
    for (int i = 0; i < imgtrans.cols; i++)
    {
        for (int j = 0; j < imgtrans.rows; j++)
        {
           /* std::cout << pow((int)imgtrans.at<Vec3b>(j, i)[2] / 255.0, parameters.gamma) << std::endl;*/
            channel[2].at<uchar>(j, i) = (uchar)(int)(pow((int)channel[2].at<uchar>(j, i) / 255.0, parameters.gamma) * 255);
        }
    }


    merge(channel, imgtrans);
    cvtColor(imgtrans, imgtrans, CV_HSV2RGB);
    if (debug)
    {
        displayImage("V", channel[2]);
        displayImage("H", imgtrans);
    }
